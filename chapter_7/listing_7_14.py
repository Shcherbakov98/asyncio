...
class LoadTester(Tk):
    # в конструкторе инициализируем поля ввода, метки, кнопку отправки и индикатор кода выполнения
    def __init__(self, loop, *args, **kwargs):
        ...
        # при нажатии на кнопку, вызывается метод _start
        self._submit = ttk.Button(self, text="Submit", command=self._start)
        self._submit.grid(column=2, row=1)

        self._pg_label = Label(self, text="Progress:")
        self._pg_label.grid(column=0, row=3)

        self._pb = ttk.Progressbar(self, orient="horizontal", length=200, mode="determinate")
        self._pb.grid(column=1, row=3, columnspan=2)

    # Устанавливает процент заполненности индикатора 0...100.
    # его следует вызывать только из главного потока
    ...
    # этот метод является обратным вызовом, который передается нагрузочному тесту
    # он добавляет обновление индикатора в очередь
    ...
    # если получилось, то обновить индикатор
