"""Конкурентное шифрование текста"""
import asyncio
import random
import string
import time
...
async def encrypt(text: str) -> bytes:
    program = ['gpg', '-c', '--batch', '-passphrase', '3ncryptm3', '--cipher-alog', 'TWOFISH']
...
async def main():
    text_list = [''.join(random.choice(string.ascii_letters) for _ in range(100)) for _ in range(10)]
    s = time.time()
