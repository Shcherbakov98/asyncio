...
class ChatServer(object):
    ...
    # ждать, пока клиент отправит допустимое имя пользователя;
    ...
    # после подключения пользователя, уведомить всех об этом
    async def _on_connect(self, username: str, writer: StreamWriter):
        writer.write(f'Добро пожаловать! Число подключенных пользователей {len(self._username_to_writer)}!\n'.encode())
        await writer.drain()
    ...
    # прослушивать сообщения от клиента, рассылать их всем остальным клиентам
    ...
    # отправить сообщения всем подключенным клиентам и удалить отключившихся клиентов
