...
TASKS_KEY = 'order_task'


async def process_order_worker(worker_id: int, queue: Queue):  # выбрать заказ из очереди и обработать его
    while True:
...
async def create_order_queue(app: Application):  # создать очередь, на 10 элементов и 5 задач-исполнителей
...
async def destroy_queue(app: Application):  # ждать завершения работающих задач
